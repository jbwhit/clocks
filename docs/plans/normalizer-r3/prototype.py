"""Revision-3 design experiment, not the package normalizer.

The error analysis and its independent review live alongside this file.
Use ``uv run --no-sync python`` from the existing project environment.
"""
...
def _decide_exactly(flagged, values, shifted, peak, stats=None):
    ...
    while True:
        ...
        if stats is not None:
            stats.setdefault("rounds", []).append(
                (precision, int(keep.sum()), int(undecided.sum()))
            )
...
def normalize_proto(log_weights, stats=None):
    values = np.asarray(log_weights, dtype=np.float64)
    weights, normalizer = normalize_branch(values)
    ...
    candidates &= ~zero
    if stats is not None:
        stats["n_candidates"] = int(candidates.sum())
        stats["n_flagged"] = 0
    ...
    if stats is not None:
        stats["n_corrected"] = int(correct.sum())
        stats["n_flagged"] = int(ambiguous.sum())
    if ambiguous.any():
        rounded[ambiguous] = _decide_exactly(
            values[indices[ambiguous]], values, shifted, peak, stats
        )
