...
`scripts/run_echolocation_population.py` provides four explicit operations:

1. generate the frozen manifest;
2. run or resume the exact manifest; and
3. finalize a complete result artifact; and
4. render figures from a finalized artifact without running inference.
