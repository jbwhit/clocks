"""Independent tests of the directed Decimal fallback."""
...
assert Path(inference.__file__).resolve().is_relative_to(Path.cwd().resolve())
...
def fallback(values, indexes):
    values = np.asarray(values, dtype=np.float64)
    peak = float(values.max())
    with np.errstate(over="ignore"):
        shifted = values - peak
    return p._decide_exactly(values[indexes], values, shifted, peak)


def observe_rounding_precisions(monkeypatch):
    """Record the supported nearest-even precision escalation rounds."""
    precisions = []
...
def test_hard_boundary_requires_more_than_56_digits(
    helper_step, expected_integer, monkeypatch
):
    values = HARD.copy()
    if helper_step:
    ...
    precisions = observe_rounding_precisions(monkeypatch)
    actual = fallback(values, [-1, -1])
    assert np.array_equal(actual, [math.ldexp(expected_integer, -1074)] * 2)
    assert precisions == [56, 112]
...
def test_real_gaussian_boundary_exhausts_precision_cap(monkeypatch):
