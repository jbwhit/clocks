...
from clocks import _weight_normalization as p
...
def test_rounding_regressions(values, index, quanta):
    ...
    assert p._normalize_log_weights(values)[0][index] == expected


def test_large_offset_stays_normalized():
    weights, _ = p._normalize_log_weights([1e18, 1e18, 1e18 - 744.3])
...
def test_equal_and_zero_weights(peak):
    weights, _ = p._normalize_log_weights([peak, peak, -np.inf])
    assert np.array_equal(weights, [0.5, 0.5, 0.0])
...
def test_exact_subnormal_band_against_reference():
    ...
    for _ in range(100):
        ...
        actual, _ = p._normalize_log_weights(values)
        if expected[-1] < tiny:
            tested += 1
...
def test_invalid_inputs_raise_existing_message(values):
    for normalize in (p._normalize_log_weights, p._normalize_log_weights_fast):
        with pytest.raises(
            RuntimeError,
            match="All particles have zero weight; the prior or forward model is",
        ):
            normalize(values)


def test_strict_and_fast_agree_bitwise_on_healthy_non_candidate_input():
    values = np.array([0.0, -1.0, -2.0])
    strict_weights, strict_evidence = p._normalize_log_weights(values)
    fast_weights, fast_evidence = p._normalize_log_weights_fast(values)
    assert np.array_equal(strict_weights, fast_weights)
    assert strict_evidence == fast_evidence
