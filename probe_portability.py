...
print(f"platform={platform.machine()} {platform.system()} python={sys.version.split()[0]} numpy={np.__version__}")
...
for s in range(500):
    ...
    normals.extend(float(x) for x in np.random.Generator(np.random.PCG64(s)).normal(size=3))
...
print("libm exp/log/hypot :", h([math.exp(u) for u in randoms[:200]]
