...
evolves toward the true mass. This is one fixed-seed visualization at the
closest, most favorable range, not a reliability measurement.
...
controls, and forward-model evaluation counts. Development seeds are used to
select controls by a declared ranking; only after those controls and
tolerances are frozen is a reserved certification block run once.
...
That protocol guards against tuning directly to the reported holdout, but a
fixed set of seeds is still regression and calibration evidence for those
specific simulated runs. It is not a population reliability estimate and
does not justify a universal pass probability. The physically robust trend
is the signal scaling: the differential gradient falls as $1/R^2$, while
the curvature that helps separate mass from range falls as $1/R^3$.
...
runs the development grid. A reserved certification run uses the already
frozen cell via `--seed-block 400`; protected blocks reject control overrides.
See
