...
The current default demo places the mass at two circumradii along a fixed off-axis
...
This committed GIF is the packaged default output from the corrected
contrast-space SMC release, not a re-render of the current code. Its
differential-rates panel draws the forward model evaluated at the posterior
mean; the code now plots the posterior mean of the particles' own
predictions, so that panel will change when the assets are next regenerated.
...
controls, and forward-model evaluation counts. Development seeds selected and
froze the controls and tolerances by the declared ranking. The reserved
400–411 block was then run exactly once for the corrected implementation.
...
The displayed study was regenerated directly from the certified block-400
records. The frozen cell passed 46/72 cases overall; pass counts from close to
far were 12, 12, 11, 8, 3, 0. The close range passed 12/12. The far/close
median position-standard-deviation ratio was 66.524, above the frozen factor
20 gate, and the 12/12 far-range three-standard-deviation coverage is retained
as a diagnostic rather than promoted after inspection into a new gate.

The completed protocol guards against tuning directly to the reported holdout,
but its fixed seeds provide regression and calibration evidence only for those
simulated cases. It does not estimate population reliability or justify a
universal pass probability. The
...
runs the development grid. The reserved run used the scenario-default cell via
`--seed-block 400`; protected blocks reject control overrides. The development
defaults and acceptance tolerances were scientifically frozen from seeds 0–11
before the one-shot 400–411 certification. Certification and corrected asset
