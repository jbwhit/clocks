...
ROWS = [
    ...
    ("Jupiter, hovering 100,000 km out", gradient(GM_JUP, 1.0e8)),
]
lines = [
...
on the next generation of clocks. And from a chair on Earth the situation
is not merely hard but *impossible in principle*: the Earth free-falls
around the Sun, and free fall cancels the leading gradient exactly — the
equivalence principle, enforcing itself. All that survives inside the
head is the tidal residue, four further orders down. No amount of focus
recovers a signal that geometry has already removed. (Earthbound clock
networks genuinely do feel the Moon and Sun near the $10^{-17}$ level —
but through the solid-Earth tide bodily raising and lowering their clocks
in Earth's *own* field, and that ride is common mode across one head:
centering erases it.)
...
fall and free fall forfeits the signal. Hovering 100,000 km above the
cloud tops, Jupiter presses on her clocks as strongly as Earth does from
a chair: one meditative minute.
