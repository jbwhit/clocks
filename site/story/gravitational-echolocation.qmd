...
This committed GIF is a pre-remediation illustration pending certification
and regeneration, not a result from the corrected contrast-space SMC
implementation.
...
froze the controls and tolerances by the declared ranking. The untouched
400–411 block has not yet been run once for the corrected implementation.
...
replace it after the one-shot protected run; its current points must not be
read as corrected-SMC evidence.
...
cell via `--seed-block 400`; protected blocks reject control overrides. The
development defaults and acceptance tolerances are scientifically frozen from
seeds 0–11, while the one-shot 400–411 certification and corrected assets
remain pending.
See
