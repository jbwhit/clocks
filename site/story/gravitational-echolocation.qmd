...
absolute [evidence normalization](../method/the-particle-filter.qmd#sec-evidence) shifts, and nothing on this page uses it.)
...
observations. The camera nearly completes an orbit while the particle cloud — six
...
and its confidence is honest: the truth lands inside all four componentwise
±3 posterior-standard-deviation intervals in all twelve runs. This is the
near field, and the sense works.
...
truth falls back inside the componentwise 3-sigma intervals nine and then
eleven times out of twelve. The head no longer knows *where*; but it has
...
So the sense is sharp only out to about two circumradii; by roughly 2.6
circumradii it is already unreliable. Just beyond that lies a band where
the degeneracy makes the head misjudge range *with
